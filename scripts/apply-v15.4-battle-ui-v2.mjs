...
function rr(text, re, replacement, label) {
  if (!re.test(text)) throw new Error(`No se encontró ${label}`);
  return text.replace(re, replacement);
}

edit('src/scenes/BootScene.ts', (text) => {
  text = text.replace("import { V15TestRosterService } from '../systems/testing/V15TestRosterService';", "import { V15TestRosterService } from '../systems/testing/V15TestRosterService';\nimport { BATTLE_UI_ATLAS_DATA_URI, BATTLE_UI_FRAMES } from '../ui/battle/v2/assets';");
  text = text.replace("    this.load.image('bandle-bg', './assets/world/regions/bandle-city/zones/portal-clearing/overworld.png');", "    this.load.image('battle-ui-v2', BATTLE_UI_ATLAS_DATA_URI);\n    this.load.image('bandle-bg', './assets/world/regions/bandle-city/zones/portal-clearing/overworld.png');");
  text = text.replace("    this.textures.get('bandle-bg').setFilter(Phaser.Textures.FilterMode.NEAREST);", "    this.textures.get('battle-ui-v2').setFilter(Phaser.Textures.FilterMode.NEAREST);\n    const battleUiTexture = this.textures.get('battle-ui-v2');\n    for (const [frameName, frame] of Object.entries(BATTLE_UI_FRAMES)) {\n      battleUiTexture.add(frameName, 0, frame.x, frame.y, frame.w, frame.h);\n    }\n\n    this.textures.get('bandle-bg').setFilter(Phaser.Textures.FilterMode.NEAREST);");
  text = text.replace("this.registry.set('app.version', '15.3.1 TEST');", "this.registry.set('app.version', '15.4 TEST');");
  return text;
});
...
"private actionObjects: Array<Phaser.GameObjects.Image | Phaser.GameObjects.Rectangle | Phaser.GameObjects.Text> = [];"
  ...
  text = text.replace("this.playerSprite = this.add.image(126, 180, playerTexture).setOrigin(0.5, 1);", "this.playerSprite = this.add.image(118, 153, playerTexture).setOrigin(0.5, 1);");
  text = text.replace("this.wildSprite = this.add.image(402, 121, wildTexture).setOrigin(0.5, 1);", "this.wildSprite = this.add.image(386, 109, wildTexture).setOrigin(0.5, 1);");
  ...
  private createSkillActionButton(x: number, y: number, skill: SkillDefinition, slot: ActiveSkillSlot, rank: number, effectivenessGlyph: string, onClick: () => void, disabled = false): void {
    ...
    const name = UiKit.label(this, x + 38, y + 6, skill.name.toUpperCase(), fontSize, disabled ? UI.text.muted : UI.text.primary, true)
      .setOrigin(0.5, 0).setAlign('center').setWordWrapWidth(64, true).setDepth(730);
    if (skill.affinityId) {
    ...
    this.actionObjects.push(card, name, glyph);
  }
...
`;
  ...
  text = rr(text, /  private createPanels\(\): void \{[\s\S]*?\n  private skillEffectTags/, panelBlock + '  private skillEffectTags', 'panel/action block');

  text = rr(text, /  private refreshUi\(\): void \{[\s\S]*?\n  \}\n\n  private updateHpUi/, `  private refreshUi(): void {
  ...
  private statusSymbol`, 'renderStatusIcons');

  text = rr(text, /  private renderCombatantMarker\(actor: BattleActor, width: number, height: number, groundY: number\): void \{[\s\S]*?\n  \}\n\n  private affinityLabelFor/, `  private renderCombatantMarker(actor: BattleActor, width: number, height: number, groundY: number): void {
  ...
  private affinityLabelFor`, 'combat marker');

  text = text.replace("const baseY = actor === 'player' ? 180 : 121;", "const baseY = actor === 'player' ? 153 : 109;");
  return text;
});

edit('package.json', (text) => text.replace('"version": "0.15.3-test.1"', '"version": "0.15.4-test.0"'));
