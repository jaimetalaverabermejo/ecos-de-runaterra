...
export class BattleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(16, 10, `¡Ha aparecido un ${wildDefinition.name} salvaje!`, {
      fontFamily: 'monospace', fontSize: '15px', color: '#ffffff', backgroundColor: '#00000088', padding: { x: 6, y: 4 }
    });

    this.add.ellipse(116, 224, 130, 24, 0x000000, 0.17);
    this.add.ellipse(width - 106, 149, 100, 20, 0x000000, 0.17);

    // SVG fallback fixes the missing/black rear Garen seen on iPad in v3.
    this.add.image(116, 220, 'garen-battle-back').setOrigin(0.5, 1).setDisplaySize(124, 126);
    this.add.image(width - 106, 146, 'teemo-battle-front').setOrigin(0.5, 1).setDisplaySize(88, 106);
    ...
    this.add.text(16, height - 24, 'STEP 3.1 · Pulido visual · Combate real en la siguiente iteración', {
      fontFamily: 'monospace', fontSize: '9px', color: '#d9e4f5'
    });
  ...
  private drawStatPanel(x: number, y: number, title: string, stats: StatBlock): void {
    ...
    this.add.text(x + 9, y + 8, title, { fontFamily: 'monospace', fontSize: '11px', color: '#ffffff' });
    this.add.text(x + 9, y + 27, `VID ${stats.hp}  ATQ ${stats.attack}  POD ${stats.power}`, {
      fontFamily: 'monospace', fontSize: '8px', color: '#dbe7f5'
    });
  ...
  private createActionButton(x: number, y: number, width: number, height: number, label: string, onClick: () => void): void {
    const button = this.add.rectangle(x, y, width, height, 0x2f466a, 1)
      .setStrokeStyle(2, 0xa8c8ff).setInteractive({ useHandCursor: true });
    this.add.text(x, y, label, { fontFamily: 'monospace', fontSize: '10px', color: '#ffffff' }).setOrigin(0.5);
    ...
    button.on(Phaser.Input.Events.POINTER_UP, () => { button.setFillStyle(0x2f466a, 1); onClick(); });
  }
}
