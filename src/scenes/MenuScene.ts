...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.createMenuRow(330, 61, 'EQUIPO', `${this.save.party.length}/5 campeones`, '♟', true, () => this.scene.start('TeamScene'));
    this.createMenuRow(330, 89, 'COLECCIÓN', `${collectionCount} Ecos`, '◇', false, () => this.setStatus('Colección preparada para una siguiente iteración.'));
    this.createMenuRow(330, 117, 'BOLSA', `${inventoryCount} objetos`, '▣', false, () => this.scene.start('BagScene'));
    this.createMenuRow(330, 145, 'MISIONES', activeQuests > 0 ? `${activeQuests} activa${activeQuests > 1 ? 's' : ''}` : `${completedQuests} completadas`, '!', false, () => this.scene.start('JournalScene'));
    this.createMenuRow(330, 173, 'TALLER', 'Recetas y crafteo', '✦', false, () => this.scene.start('CraftingScene'));
    this.createMenuRow(330, 201, 'MAPA', 'Runaterra · Bandle', '⌖', false, () => this.scene.start('WorldMapScene'));
    this.createMenuRow(330, 229, 'GUARDAR', 'Partida actual', '◆', false, () => {
  ...
  private createMenuRow(x: number, y: number, title: string, subtitle: string, icon: string, selected: boolean, onClick: () => void): void {
    ...
    UiKit.label(this, x + 17, y + 2, icon, '13px', selected ? UI.text.gold : UI.text.accent, true).setOrigin(0.5, 0);
    UiKit.label(this, x + 36, y + 1, title, UI.font.tiny, UI.text.primary, true);
    UiKit.label(this, x + 36, y + 12, subtitle, UI.font.tiny, UI.text.secondary);
    UiKit.label(this, x + 153, y + 2, '›', '15px', selected ? UI.text.gold : UI.text.muted, true).setOrigin(0.5, 0);
